...
let cart = [];
let tableNumber = "Unbekannt";

document.addEventListener("DOMContentLoaded", () => {
    
    // --- 1. Get Table Number from URL ---
    const params = new URLSearchParams(window.location.search);
    tableNumber = params.get('table') || "Unbekannt";
    
    const cartTitle = document.getElementById('cart-title');
    if(cartTitle) cartTitle.innerText = `Bestellung für Tisch ${tableNumber}`;
    ...
    orderForm.addEventListener('submit', (e) => {
        ...
        // Set the Formbold title
        document.getElementById('form-title-input').value = `Bestellung von Tisch ${tableNumber}`;
        ...
        fetch(orderForm.action, {
            ...
            if (response.ok) {
                const finalSummary = `Tisch: ${tableNumber}\n\n${summaryText}\nGesamtbetrag: ${total.toFixed(2)} €`;
            ...
            alert("Bestellung fehlgeschlagen. Bitte prüfen Sie Ihre Internetverbindung.");
        }).finally(() => {
            submitButton.innerText = "Jetzt bestellen";
